<script lang="ts">
	...
	export let data: SuperValidated<Infer<typeof PenghasilanSchema>>
	export let message: string | undefined

	let loading = false

	const form = superForm(data, {
		validators: zodClient(PenghasilanSchema),
		onSubmit() {
			loading = true
		},
		onResult({ result }) {
...
<form
	...
	use:enhance
>
	<Form.Field {form} name="penghasilan">
		<Form.Control>
			{#snippet children({ props })}
				<Form.Label>Masukan Data Jumlah Keluarga</Form.Label>
				<Input {...props} bind:value={$formData.penghasilan} />
			{/snippet}
		</Form.Control>
		<Form.FieldErrors />
	</Form.Field>
