...
<div class="container">
	<h2 class="title">Data Penghasilan</h2>

	<div class="table-header">
		<div class="left-toolbar">
			<Button href="/admin/penghasilan/tambah">
				<Plus class="icon" />
				<span>Tambahkan Data</span>
			</Button>
		</div>
		<div class="right-search">
			<Input type="text" placeholder="Cari Penghasilan Data" bind:value={search} />
		</div>
	</div>
	...
	<div class="table-wrapper">
		<Table.Root>
			...
			<Table.Body>
				{#if data.penghasilan.length > 0}
					{#each data.penghasilan.filter((item) => item.penghasilan
							.toString()
							.toLowerCase()
							.includes(search.toLowerCase())) as item, index (item.id)}
						<Table.Row>
							<Table.Cell class="text-center">{index + 1}</Table.Cell>
							<Table.Cell>{item.penghasilan}</Table.Cell>
								...
								{#if data.session.role === 'ADMIN'}
									<div class="actions">
										<Button
											size="sm"
											variant="outline"
											href={`/admin/penghasilan/edit?id=${item.id}`}
										>
											Edit
										</Button>
