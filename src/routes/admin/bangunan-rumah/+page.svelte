...
<div class="container">
	...
	<div class="table-header">
		<div class="left-toolbar">
			<Button href="/admin/bangunan-rumah/tambah">
				<Plus class="icon" />
		...
		<Table.Root>
			<Table.Header>
				<Table.Row>
					...
					<Table.Head>Bangunan Rumah</Table.Head>
					<Table.Head>Bobot</Table.Head>
					<Table.Head class="text-right">Aksi</Table.Head>
				</Table.Row>
			</Table.Header>
			<Table.Body>
				{#if data.bangunanRumah.length > 0}
					{#each data.bangunanRumah.filter((item) => item.bangunan
							.toString()
							.toLowerCase()
							.includes(search.toLowerCase())) as item, index (item.id)}
						<Table.Row>
							<Table.Cell class="text-center">{index + 1}</Table.Cell>
							<Table.Cell>{item.bangunan}</Table.Cell>
								...
								{#if data.session.role === 'ADMIN'}
									<div class="actions">
										<Button
											size="sm"
											variant="outline"
											href={`/admin/bangunan-rumah/edit?id=${item.id}`}
										>
											Edit
										</Button>
