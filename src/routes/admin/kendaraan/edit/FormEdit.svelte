<script lang="ts">
	...
	export let data: SuperValidated<Infer<typeof KendaraanSchema>>
	export let message: string | undefined

	let loading = false

	const form = superForm(data, {
		validators: zodClient(KendaraanSchema),
		onSubmit() {
			loading = true
		},
		onResult({ result }) {
...
<form
	...
	use:enhance
>
	<Form.Field {form} name="kendaraan">
		<Form.Control>
			{#snippet children({ props })}
				<Form.Label>Masukan Data Jumlah Keluarga</Form.Label>
				<Input {...props} bind:value={$formData.kendaraan} />
			{/snippet}
		</Form.Control>
		<Form.FieldErrors />
	</Form.Field>
	...
	<div class="space-x-2 self-end pt-4">
		<a href="/admin/kendaraan" class={buttonVariants({ variant: 'secondary' })}>Back</a>
