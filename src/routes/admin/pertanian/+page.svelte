...
<div class="container">
	<h2 class="title">Data Pertanian</h2>

	<div class="table-header">
		<div class="left-toolbar">
			<Button href="/admin/pertanian/tambah">
				<Plus class="icon" />
				<span>Tambahkan Data</span>
			</Button>
		</div>
		<div class="right-search">
			<Input type="text" placeholder="Cari Pertanian" bind:value={search} />
		</div>
	</div>
		...
		<Table.Root>
			<Table.Header>
				<Table.Row>
					...
					<Table.Head>Lahan Sawah</Table.Head>
					<Table.Head>Bobot</Table.Head>
					<Table.Head class="text-right">Aksi</Table.Head>
				</Table.Row>
			</Table.Header>
			<Table.Body>
				{#if data.pertanian.length > 0}
					{#each data.pertanian.filter((item) => item.lahanSawah
							.toString()
							.toLowerCase()
							.includes(search.toLowerCase())) as item, index (item.id)}
						<Table.Row>
							<Table.Cell class="text-center">{index + 1}</Table.Cell>
							<Table.Cell>{item.lahanSawah}</Table.Cell>
								...
								{#if data.session.role === 'ADMIN'}
									<div class="actions">
										<Button
											size="sm"
											variant="outline"
											href={`/admin/pertanian/edit?id=${item.id}`}
										>
											Edit
										</Button>
