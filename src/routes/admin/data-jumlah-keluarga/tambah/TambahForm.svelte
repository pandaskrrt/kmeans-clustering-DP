<script lang="ts">
	...
	export let data: SuperValidated<Infer<typeof JumlahKeluargaSchema>>
	export let message: string | undefined

	let loading = false

	const form = superForm(data, {
		validators: zodClient(JumlahKeluargaSchema),
		onSubmit() {
			loading = true
		},
		onResult({ result }) {
...
<form
	...
	use:enhance
>
	<Form.Field {form} name="jumlahKeluarga">
		<Form.Control>
			{#snippet children({ props })}
				<Form.Label>Masukan Data Jumlah Keluarga</Form.Label>
				<Input {...props} bind:value={$formData.jumlahKeluarga} />
			{/snippet}
		</Form.Control>
		<Form.FieldErrors />
	</Form.Field>
	...
	<div class="space-x-2 self-end pt-4">
		<a href="/admin/data-jumlah-keluarga" class={buttonVariants({ variant: 'secondary' })}>Back</a>
